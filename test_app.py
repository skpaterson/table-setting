...
def test_create_read_delete_fork(client):
    resp = client.post('/forks', data=json.dumps({'name': 'forky', 'prongs': 8}),
...
def test_create_update_delete_fork(client):
    resp = client.post('/forks', data=json.dumps({'name': 'forky2', 'prongs': 3}),
                       headers={'content-type': 'application/json'})
    assert resp.status_code == 201
    response_data = json.loads(resp.data)
    ...
    resp = client.put('/forks/%s' % fork_id, data=json.dumps({'name': 'forky2', 'prongs': 4}),
                      headers={'content-type': 'application/json'})
    assert resp.status_code == 204
    assert resp.data == b''
    resp = client.get('/forks/%s' % fork_id)
...
def test_create_multiple_forks(client):
    resp = client.post('/forks', data=json.dumps({'name': 'first fork', 'prongs': 100}),
                       headers={'content-type': 'application/json'})
    assert resp.status_code == 201
    response_data = json.loads(resp.data)
    ...
    resp = client.post('/forks', data=json.dumps({'name': 'second fork', 'prongs': 50}),
                       headers={'content-type': 'application/json'})
    assert resp.status_code == 201
    response_data = json.loads(resp.data)
...
def test_non_existing_fork_fails(client):
